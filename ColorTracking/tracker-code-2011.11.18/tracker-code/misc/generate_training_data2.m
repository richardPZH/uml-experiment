...
for tt = 1:T
  ...
  for obj = 1:NOBJECTS
    ...
    if((pos_int(1) < 1) || (pos_int(1) > WIDTH) || (pos_int(2) < 1) || (pos_int(2) > HEIGHT))
%      object_vel(1, obj) = (rand(1)) * 30;
%      object_vel(2, obj) = (rand(1) - 0.5);
      object_vel(2, obj) = 0;
      
      if(mod(obj, 2) == 0) % Left side
	object_vel(1, obj) = 15 + rand(1) * 10;
	pos(1, obj) = double(1);
	pos(2, obj) = obj * HEIGHT / (NOBJECTS + 3);
      else
	object_vel(1, obj) = -rand(1) * 10;
	pos(1, obj) = double(WIDTH);
	pos(2, obj) = obj * HEIGHT / (NOBJECTS + 3);
      end

%      % Initialise positions for next run
%      if(object_vel(1, obj) > 0)
%	pos(1, obj) = double(1);
%      else
%        pos(1, obj) = double(WIDTH);
%      end

%      pos(2, obj) = ceil(rand(1) * HEIGHT);
